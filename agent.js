...
async function analyzeAnswer(apiKey, promptText, answer, sources) {
  const factList = CONFIG.facts
    .map((f) => {
      ...
      if (f.superseded?.length) {
        line += `\n    PREVIOUSLY PUBLISHED (outdated, NOT wrong): ${f.superseded.join(' | ')}`;
      }
      if (f.note) line += `\n    NOTE: ${f.note}`;
      return line;
    })
...
Return JSON with exactly these keys:
{
  ...
  "factsOutdated": [array of {"label": string, "stated": string} for facts matching a PREVIOUSLY PUBLISHED value],
  "factsWrong": [array of {"label": string, "stated": string} for facts that are genuinely WRONG],
...
Grading facts — read carefully, this is the point of the exercise:
- factsCorrect: the answer matches the current value.
- factsOutdated: the answer matches something in that fact's PREVIOUSLY PUBLISHED list, or is clearly derived from it. These were true when published. Do NOT put them in factsWrong.
- factsWrong: the answer asserts something that is neither current nor previously published — a real error.
- Omission is never an error. If the answer simply does not mention a fact, leave it out of all three arrays.
- A rounded or approximate statement of the CURRENT value ("from about $1.1 million") counts as correct, not wrong.
- Report each fact at most once, in one array only. Do not split one claim across multiple labels.`,
      },
    ],
  });
...
function summarize(results) {
  ...
  const tally = (rows, bucket) => {
    for (const r of live) {
      for (const f of r[rows] || []) {
        const key = f.label || String(f);
        if (!bucket[key]) bucket[key] = { count: 0, examples: [] };
        bucket[key].count += 1;
        if (
          f.stated &&
          bucket[key].examples.length < 3 &&
          !bucket[key].examples.includes(f.stated)
        ) {
          bucket[key].examples.push(f.stated);
        }
      }
    }
  };
  ...
  const factErrors = {};
  tally('factsWrong', factErrors);

  const factOutdated = {};
  tally('factsOutdated', factOutdated);
  ...
  return {
    ...
    factOutdated,
    factOutdatedCount: Object.values(factOutdated).reduce((a, b) => a + b.count, 0),
    byCluster,
