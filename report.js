  ...
  <td width="25%" valign="top" style="padding:0 6px;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background:${P.cream};border:1px solid ${P.tan};">
      <tr><td style="padding:16px 14px;">
        ...
        <div style="font:400 30px/1.15 ${SERIF};color:${P.darkGreen};padding-top:6px;">${value}${deltaHtml}</div>
...
function buildReport(results, summary, previous) {
  ...
  const factSection = factRows
    ? `<table width="100%" cellpadding="0" cellspacing="0">${factRows}</table>
       <p style="font:400 13px/1.6 ${SANS};color:${P.slate};margin:12px 0 0;">
         Each row is a detail an answer engine asserted incorrectly. These
         usually trace back to a single stale listing or early press item being
         repeated. Correcting the source is more effective than publishing more.
       </p>`
    : `<p style="font:400 13px/1.6 ${SANS};color:${P.green};margin:0;">
         No contradicted facts this run. Every verifiable detail the engines
         stated matched the brief.
       </p>`;
