  ...
  if (!has('--dry-run')) {
        await pushToHub({
                ...
                summary: {
                          ...
                          ownedSourceCount: summary.ownedWins ?? 0,
                          competitorCounts: summary.competitorCounts || {},
                },
                // The hub expects the live1000-style field names; this agent uses its
                // own richer schema, so translate. The full `answer` text is deliberately
                // left out to keep the weekly payload small.
                results: results.map((r) => ({
                          prompt: r.prompt,
                          cluster: r.cluster,
                          brand_status: r.error ? 'error' : r.status,
                          position: r.position != null ? `#${r.position}` : '',
                          brand_context: r.note || '',
                          cited_sources: r.domains || [],
                          competitors_structured: (r.competitorsNamed || []).map(
                                      (name) => ({ name, status: 'mentioned' })
                                    ),
                          owned_source: !!r.ownedCitation,
                })),
        });
  }
